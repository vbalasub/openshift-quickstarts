...
## This application is using undertow web server as opposed to default Tomcat webserver utilized by Spring boot
To exclude Tomcat add the following to your maven pom.xml
```
<exclusion>
...
```
podman run -e MYSQL_USER=petclinic -e MYSQL_PASSWORD=petclinic -e MYSQL_ROOT_PASSWORD=root -e MYSQL_DATABASE=petclinic -p 3306:3306 mysql:8.0
```

or

```
podman run -e POSTGRES_USER=petclinic -e POSTGRES_PASSWORD=petclinic -e POSTGRES_DB=petclinic -p 5432:5432 postgres:15.2
```
